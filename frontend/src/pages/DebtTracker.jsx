  ...
  return (
    <MainLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-semibold text-text-main">Debt Tracker</h1>
          <p className="text-text-muted mt-1">Manage and settle your debts</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div className="bg-card rounded-xl p-6 shadow-lg">
            <p className="text-sm text-text-muted mb-2">Pending Settlements</p>
            <p className="text-2xl font-bold text-text-main">{activeDebts.length}</p>
          </div>
          <div className="bg-card rounded-xl p-6 shadow-lg">
            <p className="text-sm text-text-muted mb-2">Total Amount</p>
            <p className="text-2xl font-bold text-text-main">{formatCurrency(totalOwed)}</p>
          </div>
          <div className="bg-card rounded-xl p-6 shadow-lg">
            <p className="text-sm text-text-muted mb-2">Optimization</p>
        ...
        {activeDebts.length === 0 ? (
          ...
          <div className="bg-card rounded-xl p-6 shadow-lg">
            <h2 className="text-xl font-semibold text-text-main mb-6">Active Debts</h2>
                    ...
                    {settlingDebt === index ? (
                      <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <button
                          ...
                          disabled={submitting}
                          className="px-4 py-2 bg-primary hover:bg-accent disabled:bg-gray-400 text-white rounded-lg font-medium transition-colors"
                        >
                          {submitting ? '...' : 'Confirm'}
                        </button>
                        <button
                          ...
                          className="px-4 py-2 bg-card border border-slate-700 hover:bg-slate-800 text-text-main rounded-lg font-medium transition-colors"
                        >
                          Cancel
                        </button>
                      </div>
